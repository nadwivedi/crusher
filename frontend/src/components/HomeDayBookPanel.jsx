              ...
              <table className="w-full min-w-[860px]">
                <thead>
                  <tr className="bg-[linear-gradient(135deg,#0f766e_0%,#0d9488_38%,#0891b2_72%,#0284c7_100%)] text-white">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-[0.14em]">Time</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-[0.14em]">Material/Qty</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {recentEntries.map((entry, index) => (
                    <tr key={`${entry.refId || entry.voucherNumber || entry.type}-${index}`} className="hover:bg-slate-50">
                      <td className="px-4 py-3 text-sm font-semibold text-slate-700">{formatDate(entry.entryCreatedAt || entry.date)}</td>
                      <td className="px-4 py-3 text-sm text-slate-500">{formatTime(entry.entryCreatedAt || entry.date)}</td>
                      <td className="px-4 py-3 text-sm font-medium capitalize text-slate-700">{entry.type || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-700">{entry.partyName || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-700">{entry.materialSummary || '-'}</td>
